...
setTimeout(() => { // load all db's into memory
    ...
    var http = require('http')
        .createServer(app); // init http server
    ...
    app.get("/soa2redirect", (req, res) => {
			function s(h, t){
				res.contentType("html").send(`<script>localStorage.setItem("userHash", "${h}");localStorage.setItem("userName", "${t}");location.href = "/chat";</script>`)
			}
        if (!req.query.code) {
            return res.contentType("txt")
                .send("no code param")
        }
                    ...
                    x.json()
                        .then(tokenPair => {
                            fetch("https://oauth2.scratch-wiki.info/w/rest.php/soa2/v0/user", {
                                    headers: {
                                        authorization: "Bearer " + Buffer.from(tokenPair.access_token)
                                            .toString("base64")
                                    }
                                })
                                .then(r => {
...
s(hash, u)
    ...
    let ba = basicAuth({ // set panel location to the mod panel protected with a password
        users: {
            'moderator': process.env.MODPASSWORD || 'password'
        }, // add a single user with a password set from an env with a default of "password"
        challenge: true, // make a login appear on the client side
        realm: 'foo' // unknown?
    })
    app.get('/panel', ba, (req, res) => {
    ...
    app.get('/api/report/all', ba, (req, res) => { // define an API endpoint to get the content of all reports 
    ...
    app.post('/api/report/ban', ba, (req, res) => { // define an API endpoint to ban users
    ...
    app.post('/api/report/delete', ba, (req, res) => { // make an API endpoint for deleting messages (TODO)
    ...
    app.post('/api/report/reject', ba, (req, res) => { // make an API endpoint for rejecting (removing) reports
        if (modpanels.includes(req.headers.referer)) { // authenticate via the origin of the request
            var index = reportList.find(x => x.id == req.body.id); // Find the content of the used report
            index = reportList.indexOf(index); // get the location of the report
            if (index > -1) { // if the report was found
    ...
    io.on('connection', (socket) => { // handle a user connecting
                        ...
                        userDb.find({
                            ...
                            if (doc[0]) {
                                ...
                                if (object.hash == aesEncrypt(hashFromDb)) {
                                    if (object.room in whoIsOnline && !whoIsOnline[object.room].map(JSON.stringify)
                                        .includes(JSON.stringify({
                                            user: object.user
                                            , socketID: object.socket
                                        }))) {
                                        whoIsOnline[object.room].push({
                                            user: object.user
                                            , socketID: object.socket
                                        });
                                    }
                                    io.to(currentRoom)
                                        .emit('botMessage', "🎉 Welcome <b>" + object.user + "</b> to the <b>" + currentRoom + "</b> room! 🎉"); // emit a welcome message with the Modchat bot
                                } else {
                                    io.to(socket.id)
                                        .emit('kick');
                                    io.close();
        ...
        socket.on('disconnect', () => { // handle user disconnecting from the server
            userDb.find({
                ...
                if (docs[0] !== undefined) {
                    io.to(currentRoom)
                        .emit('botMessage', "😐 User <b>" + docs[0].user + "</b> left the <b>" + currentRoom + "</b> room."); // emit a welcome message with the Modchat bot
        ...
        socket.on('image', (msg) => {
            ...
            // console.log(image);
            let fileTitle = cryptoRandomString({
                length: 10
                , type: 'alphanumeric'
            }) + "." + msg.extension;
            ...
            fs.writeFile(path, buf, 'binary', function (err) {
                if (err) {
                    console.log(err);
                    io.to(socket.id)
                        .emit('botMessage', 'Error: failed writing file');
                } else {
                    app.get(path, (req, res) => {
                        res.sendFile(path);
                    })
                    console.log('File saved at ' + path);
                    console.log(`Moderating at ${"https://modchat.micahlindley.com/temp/" + fileTitle}`)
                    io.to(socket.id)
                        .emit('botMessage', 'moderating your image...');
                    fetch(`https://imgmod.micahlt.repl.co/api/image/classify?url=${process.env.DOMAIN_RUNNING + '/temp/' + fileTitle}`)
                        ...
                        .then((json) => {
                            ...
                            if (json) {
                                function test() {
                                    let indexNeutral = json.findIndex(item => {
                                        return item.className == 'Neutral';
                                    })
                                    let indexDrawing = json.findIndex(item => {
                                        return item.className == 'Drawing';
                                    })
                                    if (json[indexNeutral].probability > 0.67) {
                                        console.log('Neutral image');
                                        return true;
                                    } else if (json[indexDrawing].probability > 0.67) {
                                ...
                                if (test()) {
                                    io.to(socket.id)
                                        .emit('botMessage', 'uploading your image...');
                                    imgbb.upload(image)
                                        .then((data) => {
                                            ...
                                            userDb.find({
                                                ...
                                                if (docs[0] !== undefined) {
                                                    ...
                                                    io.to(socket.id)
                                                        .emit('botMessage', `You haven't sent any messages!  Please do so before sending images.`);
                                                }
                                            })
                                        });
                                } else {
                                    io.to(socket.id)
                                        .emit('botMessage', `That image didn't pass through our filter.  Please make sure you're sending an image that is not objectionable and is appropriate for all ages!`);
                                }
                            } else {
                                ...
                                case 1007:
                                    io.to(socket.id)
                                        .emit('botMessage', 'ERR: URL not accessible or malformed image');
                                    break;
                                case 1002:
                                    io.to(socket.id)
                                        .emit('botMessage', 'ERR: Invalid URL');
                                    break;
                                case 1008:
                                    io.to(socket.id)
                                        .emit('botMessage', 'ERR: File size too large');
                                    break;
                                default:
                                    io.to(socket.id)
                                        .emit('botMessage', 'ERR: Unknown');
                                    break;
                                }
    ...
    var sendMessage = (room, msg, sender, document, socketIdd) => {
        ...
        default: {
            if (!filter.isProfane(msg.replace(String.fromCharCode(8203), ''))) { // checks if message doesn't contain rude words
                if (msg.length > 250) {
                    io.to(socketIdd)
                        .emit('botMessage', 'Do not bypass the char limits!  This is a warning!');
                } else {
                    ...
                    io.to(room)
                        .emit('chatMessage', { // emit the message to all clients in the room
                            "message": msg
                            , "raw_message": rawMessage
                            , "sender": sender, // set the sender to the sender's username
